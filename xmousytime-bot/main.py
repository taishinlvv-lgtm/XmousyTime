...
def get_staff(message):
    ...
    if any(w in message_lower for w in ["会計", "経費", "請求", "お金", "収支", "税", "財務"]):
        return "和大"
    elif any(w in message_lower for w in ["法律", "契約", "規約", "法的", "弁護"]):
        return "ひかり"
    elif any(w in message_lower for w in ["サイト", "コード", "開発", "プログラム", "ウェブ", "web"]):
        return "セバス"
    elif any(w in message_lower for w in ["新規", "営業", "提案書", "アプローチ", "開拓"]):
        return "はやと"
    elif any(w in message_lower for w in ["問い合わせ", "見積", "クロージング", "対応", "返信"]):
        return "晴"
    elif any(w in message_lower for w in ["sns", "集客", "フォロワー", "投稿", "インスタ", "twitter"]):
        return "ひびき"
    else:
        return "エリ"
...
def callback():
    ...
    for event in body.get("events", []):
        ...
        if event.get("type") == "message" and event["message"].get("type") == "text":
            ...
            # IDを教えてコマンド
            if "myid" in user_message.lower() or "マイid" in user_message.lower():
                reply_message(reply_token, f"あなたのUser IDは：\n{user_id}")
                continue
            ...
            try:
                ...
                reply_message(reply_token, "エラーが発生しました。")
    return "OK"
